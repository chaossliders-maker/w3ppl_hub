...
Existing projects: ${JSON.stringify(body.existingProjects?.slice(0, 20))}`;
  ...
  "investorType": one of ["angel","micro-vc","vc","crypto-fund","family-office","syndicate","dao","ecosystem","corporate-vc","hni"],
  "description": string (investment strategy, focus, value-add beyond capital),
  "status": "open"|"paused"|"funded"|"cancelled",
  "minTicket": number (USD, minimum check size, can be as low as 0.10),
  "maxTicket": number (USD, 0 = unlimited),
  "preferredInstruments": string[] from ["saft","safe","safe-warrant","equity","token","convertible","otc","grant","revenue-share","lp","any"],
  "preferredStages": string[] from ["pre-idea","angels","pre-seed","seed","strategic","private-sale","series-a","series-b","pre-tge","tge","public-sale","otc","secondary","grants","ecosystem-fund","hackathon"],
  "preferredVerticals": string[] from [${TAG_LIST}],
  "preferredGeos": string[] from ["us","uk","eu","sg","hk","asia","dubai","mena","latam","cis","africa","global","cayman","swiss"],
  "maxCliffMonths": number|null (max cliff they accept, null = no preference),
  "minVestingMonths": number|null,
  "maxVestingMonths": number|null,
  "minTgeUnlockPct": number|null (minimum % unlocked at TGE they require),
  "targetRoiX": number|null (target return multiple, e.g. 10 for 10x),
  "coInvestOk": boolean,
  "leadOnly": boolean (true = only leads rounds),
  "kycOk": boolean (willing to do KYC/AML),
  "proRataReq": boolean (requires pro-rata rights),
  "terms": string (any additional criteria, thesis, co-invest notes),
  "tags": string[] from [${TAG_LIST}],
  "subEntities": [],
  "dedup": null
}`
      : `{
  "title": string (descriptive: "ProjectName — RoundLabel · $Amount"),
  "projectName": string,
  "round": "pre-idea"|"angels"|"pre-seed"|"seed"|"strategic"|"private-sale"|"series-a"|"series-b"|"pre-tge"|"tge"|"public-sale"|"otc"|"secondary"|"grants"|"ecosystem-fund"|"hackathon",
  "status": "draft"|"open"|"in-review"|"dd-active"|"term-sheet"|"signed"|"funded"|"paused"|"closed-lost"|"cancelled",
  "amount": number (USD target raise, use midpoint if range given, 0 if unknown),
  "currency": string (default "USD"),
  "token": string (token ticker if exists),
  "tokenType": "utility"|"governance"|"revenue"|"payment"|"nft"|"rwa"|"none"|null,
  "instrument": "saft"|"safe"|"safe-warrant"|"equity"|"token"|"convertible"|"otc"|"grant"|"revenue-share"|"lp"|"any",
  "valuationCap": number|null (USD, for SAFE/equity rounds),
  "fdv": number|null (USD fully diluted valuation for token rounds),
  "cliffMonths": number (months of cliff, 0 if none),
  "vestingMonths": number (total vesting duration months, 0 if none),
  "tgeUnlockPct": number (% of tokens unlocked at TGE, 0-100),
  "description": string (what the project does, problem solved, market),
  "traction": string (key metrics: DAU, TVL, revenue, waitlist, etc.),
  "existingInvestors": string (names of existing investors if mentioned),
  "closeDate": string|null (ISO date if deadline mentioned),
  "deckUrl": string (URL to pitch deck if provided),
  "dataroomUrl": string (URL to data room if provided),
  "leadRequired": boolean (true if seeking a lead investor),
  "proRata": boolean (true if pro-rata rights offered),
  "kycRequired": boolean,
  "geoRestrictions": string[] (allowed investor geos, empty = global),
  "terms": string (additional terms, notes),
  "tags": string[] from [${TAG_LIST}],
  "linked": [{"type":"contact"|"company"|"project","id":string,"name":string}],
